...
import static pt.ulisboa.tecnico.socialsoftware.tutor.exceptions.ErrorMessage.INVALID_ORDER_FOR_OPTION;
...
public class Option implements DomainEntity {
    ...
    public Option(OptionDto option, boolean b) {
        setSequence(option.getSequence());
        setContent(option.getContent());
        setCorrect(option.isCorrect());
        if (b && option.isCorrect() && option.getOrder()>=1){
            setOrder(option.getOrder());
        }
    }
    ...
    public void setOrder(Integer order) {
        if (isCorrect() && order == null)
            throw new TutorException(INVALID_ORDER_FOR_OPTION);
        this.order = order;
    }
